...
def print_event(cpu, data, size):
    ...
    if event.sector != 18446744073709551615 and event.sector not in sectorMap:
        ...
        print('%-20s %-16s %-20s %-20s %-20s' % (event.name.decode('utf-8'), event.disk.decode('utf-8'), hex(event.sector), hex(event.total_data_len), event.rwflag))
